...
float4 LightPixelShader(PixelInputType input) : SV_TARGET
{
    ...
    float4 lightColor1 = diffuseColors[0] * lightIntensity1;
    float4 lightColor2 = diffuseColors[1] * lightIntensity2;
    float4 lightColor3 = diffuseColors[2] * lightIntensity3;
    float4 lightColor4 = diffuseColors[3] * lightIntensity4;

	float4 totalLightColor = saturate(lightColor1 + lightColor2 + lightColor3 + lightColor4);
	...
	// Apply lightmap
	float4 lightColor = shaderTextures[2].Sample(SampleType, input.tex);

	// Apply alpha
	float4 alphaValue = shaderTextures[3].Sample(SampleType, input.tex);
	...
	float4 blendColor = saturate( ( alphaValue * color1 ) + ( ( 1.0f - alphaValue ) * color2 ) );
	...
	if(lightIntensity > 0.0f)
    {
		...
		float4 specularIntensity = shaderTextures[5].Sample(SampleType, input.tex);
